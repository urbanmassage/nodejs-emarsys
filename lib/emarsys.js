...
Emarsys.prototype.updateEmailTemplate = function (emailId, emailData, callback) {
  this.request('/api/v2/email/' + emailId + '/patch', emailData, 'POST', callback);
};


/**
 * Lists all external event for emails
 * @param {string} eventName
 * @param {function} callback
 */
Emarsys.prototype.getExternalEvents = function (callback) {
  this.request('/api/v2/event', null, 'GET', callback);
};


/**
 * Creates an external event for emails
 * @param {string} eventName
 * @param {function} callback
 */
Emarsys.prototype.createExternalEvent = function (eventName, callback) {
  this.request('/api/v2/event', {name: eventName}, 'POST', callback);
};
